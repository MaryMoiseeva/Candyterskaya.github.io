...
<main>
    <section id="tort">
        ...
        $stmt = $pdo->query("SELECT products.*, cart.id as cart_id FROM cart INNER JOIN products ON products.id=cart.product_id WHERE cart.user_id = {$_SESSION['user_id']}");
        ...
        foreach ($products as $product) {
            ...
            echo "<button data-id='{$product['cart_id']}' onclick='btnClickDelete(this)'>Удалить</button>";
            echo "</div>";
            echo "</div>";
        }
        ?>
        ...
        <div id="total">
            <h3>Итого: <?php echo $totalPrice; ?> рублей</h3>
        </div>
    </section>
</main>
